...
export const getBalances = createAsyncThunk(
  ...
  async ({ address, networkID, provider }: IBaseAddressAsyncThunk) => {
    ...
    const calls = [
      { address: addresses[networkID].MUSH_ADDRESS, name: "balanceOf", params: [address] },
      { address: addresses[networkID].SMUSH_ADDRESS, name: "balanceOf", params: [address] },
      { address: addresses[networkID].WSMUSH_ADDRESS, name: "balanceOf", params: [address] },
    ];
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk, { dispatch }) => {
    try {
      const ohmContract = new ethers.Contract(
        addresses[networkID].MUSH_ADDRESS as string,
        ierc20Abi,
        provider,
      ) as IERC20;
      ...
      const shecContract = new ethers.Contract(addresses[networkID].SMUSH_ADDRESS as string, sHECv2, provider);
      const wrapAllowance = await shecContract.allowance(address, addresses[networkID].WSMUSH_ADDRESS);
      const wshecContract = new ethers.Contract(addresses[networkID].WSMUSH_ADDRESS as string, wsHEC, provider);
      const unwrapAllowance = await wshecContract.allowance(address, addresses[networkID].WSMUSH_ADDRESS);
