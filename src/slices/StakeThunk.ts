...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const ohmContract = new ethers.Contract(addresses[networkID].MUSH_ADDRESS as string, ierc20ABI, signer) as IERC20;
    const sohmContract = new ethers.Contract(addresses[networkID].SMUSH_ADDRESS as string, ierc20ABI, signer) as IERC20;
    let approveTx;
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    try {
      if (action === "stake") {
        ...
        console.log(`🚀 - stakeTx`, ethers.utils.parseUnits(value, "gwei"));
        stakeTx = await stakingHelper.stake(ethers.utils.parseUnits(value, "gwei"));
      } else {
        uaData.type = "unstake";
        console.log(`🚀 - ethers.utils.parseUnits(value, "gwei")`, ethers.utils.parseUnits(value, "gwei"));
